...
static inline void _eloopStateSetMask(struct epoll_event *event, int mask) {
	if (mask & EVT_READ)  event->events |= EPOLLIN;
	if (mask & EVT_WRITE) event->events |= EPOLLOUT;
}
...
static int eloopStateAdd(eloop *el, int fd, int mask) {
	...
	if (epoll_ctl(es->efd, op, fd, &event) == -1) return EVT_ERR;
	return EVT_OK;
}
...
static void eloopStateDelete(eloop *el, int fd, int mask) {
	...
	if (newmask != EVT_DELETE) epoll_ctl(es->efd, EPOLL_CTL_ADD, fd, &event);
	else epoll_ctl(es->efd, EPOLL_CTL_DEL, fd, &event);
}
...
static int eloopPoll(eloop *el) {
	...
	if (fdcount > 0) {
		for (int i = 0; i < fdcount; ++i) {
			...
			if (evts & EPOLLIN)  newmask |= EVT_READ;
			if (evts & EPOLLOUT) newmask |= EVT_WRITE;
			if ((evts & EPOLLERR) || (evts & EPOLLHUP)) newmask |= (EVT_READ|EVT_WRITE);
