...
int inetSetSocketNonBlocking(int sockfd) {
...
static int inetCleanupAfterFailure(int sockfd, struct addrinfo *servinfo) {
    if (sockfd != -1)
        (void)close(sockfd);
    warning("INET ERR: %s\n", strerror(errno));
...
static inline int getaddrinfoError(int err) {
...
static int _inetCreateServer(int port, char *bindaddr, int addrfam,
...
int _inetAccept(int sockfd, int nonBlocking) {
    int acceptedfd;
    struct sockaddr_storage in_addr;
    socklen_t socklen;

    if ((acceptedfd = accept(sockfd, (struct sockaddr *)&in_addr,
                    &socklen)) ==-1)
        return INET_ERR;
